...
export const paymentController = {
    ...
    async createCheckoutSession(req, res) {
        ...
        const orderItemsWithOwner = await Promise.all(cartItems.map(async (cartItem) => {
    ...
    async handleWebhook(req, res) {
        ...
        if (event.type === "checkout.session.completed") {
            ...
            await productStore.decrementStock(order.items.map((item) => ({
                productId: item.productId,
                quantity: item.quantity,
            })));
            ...
            // Email shopkeepers
            const shopkeeperIds = [
                ...new Set((await Promise.all(order.items.map(async (item) => {
            ...
            if (shopkeeperIds.length > 0) {
                ...
                const recipientEmails = shopkeepers
                    .filter((shopkeeper) => Boolean(shopkeeper))
                    .filter((shopkeeper) => shopkeeper.role === "shopkeeper")
                    .map((shopkeeper) => shopkeeper.email);
                await Promise.allSettled(recipientEmails.map((email) => sendEmail({
                    to: email,
    ...
    async cancelPaidOrder(req, res) {
        ...
        if (!order) {
            throw new AppError("Order not found, already cancelled, or not eligible for cancellation", 404);
        }
        ...
        await productStore.restoreStock(order.items.map((item) => ({
            productId: item.productId,
            quantity: item.quantity,
        })));
        // Send notifications
