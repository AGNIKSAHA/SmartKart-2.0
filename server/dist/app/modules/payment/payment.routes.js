...
paymentRouter.post("/create-checkout-session", requireAuth, requireRole("consumer"), validateBody(createOrderSchema), paymentController.createCheckoutSession);
...
paymentRouter.get("/session-status", requireAuth, paymentController.getSessionStatus);
// Cancel a paid order (Stripe refund + stock restore)
paymentRouter.patch("/:orderId/cancel", requireAuth, requireRole("consumer"), paymentController.cancelPaidOrder);
