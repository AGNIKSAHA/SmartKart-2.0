...
export const RegisterPage = () => {
  ...
  // prompt: 'select_account' forces a fresh token — prevents 401 on re-login.
  const handleGoogleLogin = useGoogleOAuthLogin({
    onSuccess: (tokenResponse) => {
      setGoogleTokenResponse({ access_token: tokenResponse.access_token });
      setRole("");
    },
  ...
  const onConfirmGoogleRole = async () => {
    ...
    try {
      ...
      setGoogleTokenResponse(null); // Reset to allow retry
    }
  };
